...
def LMS(xn,dn,M,mu,itr):
    ...
    for k in range(itr)[M-1+2:itr]:
        x=inVector(xn,k,k-M+1)
        y=multiVector(W[k-1-5],x)
        en[k]=dn[k]-y
        x=inVector(xn,k-2,k-M+1-2)
        W[k]=np.add(W[k-1],2*mu*en[k-1-2]*x)
...
if __name__ == '__main__':
    ...
    plt.legend()
    plt.figure(4)
    plt.plot(t,xn,'k-',label='$Input$')
    plt.plot(t,yn,'r-',label='$Output$')
    plt.plot(t,dn,'c-',label='$Desired$')
    plt.plot(t,en,'y-',label='$Error$')
    plt.text(40,20,'@Author:Xiao Jiqing\n@Date:May 10th,2014')
    plt.title("DLMS Result")
    plt.annotate('$D_3=D_4=2,D_1=5$',xy=(10,10),xytext=(40,15),arrowprops=dict(facecolor='black',shrink=0.05))
    plt.ylim(-25,25)
    plt.legend()
    plt.show()
##    main()
